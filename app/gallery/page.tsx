...
export default function GalleryPage() {
  ...
  useEffect(() => {
    ...
    if (query !== curr) {
      ...
      // hapus param lama ?q= biar konsisten
      sp.delete("q");
  ...
  async function onDelete(it: GalleryItem) {
    ...
    if (!token)
      return alert("Delete token not found. Use the same browser you used to submit.");
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
          {filtered.map((it) => {
            ...
            return (
              <div
                key={it.id}
                className="bg-white/5 rounded-2xl overflow-hidden shadow-lg backdrop-blur-md"
              >
                {/* gambar + LIKE overlay */}
                ...
                <div className="p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Link href={searchLink} className="btn px-3 py-1 text-xs">
                      Search on Gallery
                    </Link>
                    {openPost && (
                      <a
                    ...
                    {/* tombol like lama DIHAPUS (dipindah ke overlay) */}
                  </div>
