...
export async function POST(req: Request) {
  try {
    ...
    // 2) Simpan metadata (hash token, bukan token asli)
    ...
    const meta = {
      ...
      x,
      discord,
      url: image.url,     // URL gambar publik
      createdAt,
      ownerTokenHash,     // hanya hash disimpan di server
    };
    ...
    // Respons ke client: kasih token asli (untuk owner delete) + metaUrl untuk admin/owner
    return NextResponse.json({
      success: true,
      id,
      url: image.url,
